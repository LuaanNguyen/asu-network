...
export default async function ProfilePage({ params }: ProfilePageProps) {
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-6 py-14 sm:px-8">
      <Link
        href="/people"
        className="mb-6 inline-flex rounded-full border border-line px-4 py-2 text-xs lowercase tracking-[0.14em] text-muted transition hover:bg-surface"
      >
        Back to directory
      </Link>

      <section className="mb-6 rounded-2xl border border-line bg-surface p-6 sm:p-8">
        <p className="mb-2 font-mono text-xs lowercase tracking-[0.2em] text-muted">{person.program}</p>
      ...
      <section className="mb-6 grid gap-6 rounded-2xl border border-line bg-surface p-6 sm:grid-cols-2">
        <div>
          <p className="mb-2 font-mono text-xs lowercase tracking-[0.16em] text-muted">Focus Areas</p>
          <ul className="flex flex-wrap gap-2">
            {person.focusAreas.map((focus) => (
              <li
                key={focus}
                className="rounded-full border border-line px-3 py-1.5 font-mono text-xs lowercase tracking-[0.12em]"
              >
                {focus}
              </li>
            ))}
        ...
        <div>
          <p className="mb-2 font-mono text-xs lowercase tracking-[0.16em] text-muted">Links</p>
      ...
      <section className="rounded-2xl border border-line bg-surface p-6">
        <p className="mb-3 font-mono text-xs lowercase tracking-[0.16em] text-muted">Connected People</p>
          ...
          {connected.map((entry) => (
            <li key={entry.id} className="rounded-xl border border-line/70 p-4">
              ...
              <Link
                ...
                className="mt-3 inline-flex text-xs font-semibold lowercase tracking-[0.1em] text-accent-ink"
              >
                View Profile
              </Link>
            </li>
